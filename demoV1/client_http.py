...
def client_init(url="localhost:40183",
...
def infer(triton_client, model_name,
    ...
    inputs.append(httpclient.InferInput(input0, [1, 1, 128, 160, 112], "FP32"))

    # Initialize the data
    # np.random.seed(2022)
    inputs[0].set_data_from_numpy(np.random.random([1, 1, 128, 160, 112]).astype(np.float32), binary_data=False)
...
if __name__ == '__main__':
    ...
    print("http infer: {}".format(time.time() - s))